 ...
 *  - Tap the FAB → [onNewOrder] → navigate to the newly created order's detail.
 *
 ...
 * stays in [ProjectDetailViewModel] rather than being duplicated here.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun AddToOrderScreen(
    projectId: String,
    viewModel: AddToOrderViewModel,
    onNavigateBack: () -> Unit,
    /** Creates a new order from the selected beads; returns the new orderId or null on failure. */
    onNewOrder: suspend () -> String?,
    /** Appends the selected beads into an existing [orderId]. */
    onImportIntoOrder: suspend (orderId: String) -> Unit,
    ...
    Scaffold(
        ...
        if (eligibleOrders.isEmpty()) {
            Column(
                ...
                Text(
                    text = stringResource(R.string.no_eligible_orders),
    ...
    confirmTarget?.let { target ->
        AlertDialog(
            ...
            text = { Text(stringResource(R.string.add_to_existing_order_confirm)) },
            confirmButton = {
                TextButton(
                    onClick = {
                        confirmTarget = null
                        if (!busy) {
                            scope.launch {
                                busy = true
                                try {
                                    onImportIntoOrder(target.order.orderId)
