...
/**
 * Fetches price and availability for an Aura Crystals SKU.
 *
 * Aura Crystals runs on Shopify. Appending ".json" to any product URL returns a
 * structured JSON payload (no scraping required). The variant's [price] is a decimal
 * string; we multiply by 100 and round to cents.
 *
 * Availability: [inventoryQuantity] > 0, OR [inventoryPolicy] == "continue" (which
 * means backorders are allowed — the item ships when restocked but can be ordered now).
 */
@Singleton
class AcPackPriceFetcher @Inject constructor(
    ...
    override suspend fun fetch(url: String): ScrapedPack = withContext(Dispatchers.IO) {
        ...
        okHttpClient.newCall(request).execute().use { response ->
            if (!response.isSuccessful) throw IOException("AC HTTP ${response.code} for $jsonUrl")
            val body = response.body?.string() ?: throw IOException("AC empty response body")
